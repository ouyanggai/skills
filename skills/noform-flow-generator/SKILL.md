...
- 生成或修改完成后，运行 [scripts/validate_noform_component.py](scripts/validate_noform_component.py) 做宿主约定校验：
...
8. 验证一致性。
   先跑 `inspect_noform_host.py` 看整体盘点，再跑 `validate_noform_component.py` 检查组件名、配置、API 映射、关键方法、中文校验提示和流程挂载是否符合预期。
